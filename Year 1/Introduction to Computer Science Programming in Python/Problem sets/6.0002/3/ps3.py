...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {(x, y): dirt_amount for x in range(math.floor(width)) for y in range(math.floor(height))}
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return sum(1 for pos in self.tiles if self.tiles[pos] == 0)
    ...
    def is_position_in_room(self, pos):
        ...
        """
        pos_x, pos_y = math.floor(pos.get_x()), math.floor(pos.get_y())
        return True if self.tiles.get((pos_x, pos_y), None) is not None else False
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        while not self.room.is_position_valid(self.pos):
            self.pos = Position(room.width * random.random(), room.height * random.random())
    ...
    def set_robot_direction(self, direction):
        ...
        if direction >= 0 and direction <= 360:
            ...
            raise ValueError('Direction must be between 0 to 360 degree')
...
class FurnishedRoom(RectangularRoom):
    ...
    def add_furniture_to_room(self):
        ...
        Furniture location and size is randomly selected. Width and height are chosen
    ...
    def is_position_furnished(self, pos):
        ...
        """
        pos_x, pos_y = math.floor(pos.get_x()), math.floor(pos.get_y())
        return True if (pos_x, pos_y) in self.furniture_tiles else False
    ...
    def is_position_valid(self, pos):
        ...
        """
        if super().is_position_in_room(pos) and not self.is_position_furnished(pos):
            return True
        return False
    ...
    def get_num_tiles(self):
        ...
        """
        return sum(1 for pos in self.tiles if pos not in self.furniture_tiles)
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room and not in a furnished area).
        """
        pos_x, pos_y = self.width*random.random(), self.height.random.random()
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        exp_pos = self.pos.get_new_position(self.dir, self.speed)
        if self.room.is_position_valid(exp_pos):
            self.pos = exp_pos  # move to tile
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        exp_pos = self.pos.get_new_position(self.dir, self.speed)
        if self.room.is_position_valid(exp_pos) and not self.gets_faulty():
            self.pos = exp_pos  # move to tile
            self.room.clean_tile_at_position(self.pos, self.cap)
        else:
            self.set_robot_direction(360 * random.random())
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount,
    ...
    dirt_amount: an int
    min_coverage: a float (0 <= min_coverage <= 1.0) (= n_cleaned_tiles/n_tiles)
    ...
    for trial in range(num_trials):
        ...
        robots = [robot_type(room, speed, capacity) for n in range(num_robots)]
        anim = ps3_visualize.RobotVisualization(num_robots, width, height,
                                                type(room)==FurnishedRoom, delay=.01)
        while room.get_num_cleaned_tiles()/room.get_num_tiles() < min_coverage:
            for robot in robots:
                robot.update_position_and_clean()
            anim.update(room, robots)
            n_time_steps += 1
        sum_n_time_steps += n_time_steps

    anim.done()
    return sum_n_time_steps/num_trials
